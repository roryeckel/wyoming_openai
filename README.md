...
4. **Streaming Compatibility**: Bridge Wyoming's streaming TTS protocol with OpenAI-compatible APIs through intelligent sentence boundary chunking powered by [yasbd](https://github.com/speedyk-005/yasbd-lib), enabling responsive incremental audio delivery even when the underlying API doesn't support streaming text input. Concurrent pipelining (default limit of three in-flight requests) keeps playback smooth while ensuring events remain ordered.
...
| `--stt-strip-regex`                     | `STT_STRIP_REGEX`                          | None                                          | Optional regular expression removed from STT transcripts before they are sent to the client (e.g. to strip CoT/reasoning tags). Compiled with `re.DOTALL` so `.` spans newlines, and the cleaned result is trimmed. Applied to the final transcript (non-streaming, streaming, and realtime). Invalid or empty values are a startup error. |
...
`STT_STRIP_REGEX` is applied to the final transcript only, and is the simplest hook for cleaning up model chatter (e.g. stripping `<cot>…</cot>` blocks). Because it runs after the full transcript is assembled, it works the same way across the streaming, non-streaming, and realtime paths.
...
- **`major.minor.patch version`**: Specific version tags (e.g., `0.6.1`) correspond to specific stable releases of the Wyoming OpenAI proxy server. These tags are ideal for users who need a consistent, reproducible environment and want to avoid breaking changes introduced in newer versions.

- **`major.minor version`**: Tags that follow the `major.minor` format (e.g., `0.6`) represent a range of patch-level updates within the same minor version series. These tags are useful for users who want to stay updated with bug fixes and minor improvements without upgrading to a new major or minor version.
